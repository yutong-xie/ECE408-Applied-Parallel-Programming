...
__global__ void convlayer_forward(float *y, float *x, float *w, int B, int C, int H, int K, int W, int M)
{
...
__global__ void convlayer_forward2(float *y, float *x, float *w, int B, int C, int H, int K, int W, int M)
{

    __shared__ float MaskTile[TILE_WIDTH2][TILE_WIDTH2];
    __shared__ float InputTile[TILE_WIDTH2][TILE_WIDTH2];
    ...
    float acc = 0.0;
    int num_iterations = ceil(unrollColumn/(1.0*TILE_WIDTH2));

    for (int i = 0; i < num_iterations; i++) {
      int lx = i*TILE_WIDTH2 + tx;
      int ly = i*TILE_WIDTH2 + ty;
      ...
      //      + MaskTile[ty][14] * InputTile[14][tx]
      //      + MaskTile[ty][15] * InputTile[15][tx]
      //      + MaskTile[ty][16] * InputTile[16][tx]
      //      + MaskTile[ty][17] * InputTile[17][tx]
      //      + MaskTile[ty][18] * InputTile[18][tx]
      //      + MaskTile[ty][19] * InputTile[19][tx]
      //      + MaskTile[ty][20] * InputTile[20][tx]
      //      + MaskTile[ty][21] * InputTile[21][tx]
      //      + MaskTile[ty][22] * InputTile[22][tx]
      //      + MaskTile[ty][23] * InputTile[23][tx];
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    // printf("M,C,H,K,W is : %d,%d,%d,%d,%d",M,C,H,K,W);
      convlayer_forward2<<<gridDim, blockDim>>>(y.dptr_,x.dptr_,w.dptr_,B,C,H,K,W,M);
    }else{
      dim3 blockDim(TILE_WIDTH, TILE_WIDTH, 1);
      dim3 gridDim(ceil(H_out*W_out/(1.0*TILE_WIDTH)), ceil(M/(1.0*TILE_WIDTH)), B);
    // Call the kernel
    // printf("M,C,H,K,W is : %d,%d,%d,%d,%d",M,C,H,K,W);
      convlayer_forward<<<gridDim, blockDim>>>(y.dptr_,x.dptr_,w.dptr_,B,C,H,K,W,M);
    }
